...
import { PROFILE_PATH, SIGNIN_PATH } from '../../../constants/routes';
...
const CommentHeader = styled(Flex, transientOptions)`
	...
	background: ${({ $adopted }) =>
		$adopted ? '#238BE680' : 'var(--secondary-bg-color)'};
`;
...
const Comment = ({
	...
	const handleClickAdopt = (adopted) => () =>
		toggleAdoptedMutate({ commentId: id, adopted });
					...
					<Flex direction="column" w="100%">
						<Flex gap="10px">
							...
							<Flex ml="auto" gap="10px">
								{isPostAuthor && !postInfo.completed && (
									<AdoptedButton onClick={handleClickAdopt(true)} />
								)}
								{isPostAuthor &&
									isTopComment &&
									postInfo.completed &&
									adopted && (
										<Button
											...
											radius="xl"
											color="red"
											onClick={handleClickAdopt(false)}>
											채택 취소
										</Button>
									)}
								<LikeChip
